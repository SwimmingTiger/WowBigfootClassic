...
local function OnUpdate(self, elapsed)
    ...
    elseif self.SwingStart then
        if IsPlayerMoving() or IsFalling() then
            ...
            self.texture:SetWidth(config.width * math_min(((curTime - self.SwingStart) / (SWING_TIME + movementDelay)),1))
            self:SetAlpha(config.alpha)
        end
    else
        self.texture:SetWidth(config.width * (1 - math_min(((curTime - self.lastshot) / self.swingtime),1)))
        self:SetAlpha(config.alpha)
    end
end
...
function YaHT:COMBAT_LOG_EVENT_UNFILTERED()
    ...
    if event == "SWING_DAMAGE" or event == "ENVIRONMENTAL_DAMAGE" or event == "RANGE_DAMAGE" or event == "SPELL_DAMAGE" then
        ...
        if targetID == UnitGUID("player") then
            local maxValue = CastingBarFrame.maxValue or 0
            CastingBarFrame.maxValue = maxValue + math_min(CastingBarFrame:GetValue(),AimedDelay)
