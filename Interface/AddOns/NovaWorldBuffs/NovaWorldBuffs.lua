...
function NWB:getShortBuffTimers(channel, layerID)
	...
	if (NWB.isLayered) then
		...
		if (layerID and not layer) then
			return L["That layer wasn't found or has no valid timers."];
		end
	...
	if (layerID) then
		return msg .. L[" (Layer "] .. layerID .. L[" of "] .. count .. ")";
...
function NWB:print(msg, channel, prefix)
	...
	elseif (tonumber(channel)) then
		...
		if (id == 0) then
			print(L["|cFFFFFF00No channel with id |cFFFF5100"] .. channel .. L[" |cFFFFFF00exists."]);
			print(L["|cFFFFFF00Type \"/wb\" to print world buff timers to yourself."]);
			print(L["|cFFFFFF00Type \"/wb config\" to open options."]);
			print(L["|cFFFFFF00Type \"/wb guild\" to post buff timers to the specified chat channel (accepts channel names and numbers)."]);
			print(L["|cFFFFFF00Use \"/sf\" in the same way for songflowers."]);
			print(L["|cFFFFFF00Type \"/dmf\" for your Darkmoon Faire buff cooldown."]);
			print(L["|cFFFFFF00Type \"/buffs\" to view all your alts world buffs."]);
			return;
		end
		if (not prefix) then
			printPrefix = "[NovaWorldBuffs]";
	...
	elseif (channel ~= nil) then
		...
		local id, name = GetChannelName(channel);
		if (id == 0) then
			print(L["|cFFFFFF00No channel with name |cFFFF5100" .. channel .. " |cFFFFFF00exists."]);
			print(L["|cFFFFFF00Type \"/wb\" to print world buff timers to yourself."]);
			print(L["|cFFFFFF00Type \"/wb config\" to open options."]);
			print(L["|cFFFFFF00Type \"/wb guild\" to post buff timers to the specified chat channel (accepts channel names and numbers)."]);
			print(L["|cFFFFFF00Use \"/sf\" in the same way for songflowers."]);
...
function NWB:updateWorldbuffMarkers(type, layer)
	...
	if (NWB.isLayered and layer) then
		...
		if (type == "ony") then
			...
			_G[type .. layer .. "NWBWorldMap"].fsLayer:SetText(L["|cff00ff00[Layer "] .. count.. "]");
		end
		if (NWB.data.layers[layer]) then
...
NWBlayerFrame.fs3:SetText(L["|cFFDEDE42Layers may be inaccurate for a few hours after server restarts.\n"]
		.. L["Layers will disappear from here 6 hours after having no timers."]);
...
function NWB:setCurrentLayerText(unit)
	...
	for k, v in NWB:pairsByKeys(NWB.data.layers) do
		...
		if (k == tonumber(zoneID)) then
			NWBlayerFrame.fs2:SetText(L["|cFF9CD6DEYou are currently on |cff00ff00[Layer "] .. count .. "]|cFF9CD6DE.|r");
	...
	if (not IsInGuild()) then
		...
		if (NWB.faction == "Horde") then
			NWBLayerMapFrame.EditBox:Insert(L["\n|cff00ff00[Layer "] .. count .. L["]|r  |cff9CD6DE(Orgrimmar "] .. k .. ")|r  "
					.. "|cFFFF5100(" .. zoneCount .. L[" zones mapped)|r\n"] .. text);
		else
...
MinimapLayerFrame.fs:SetText(L["Layer 1"]);
...
function NWB:recalcMinimapLayerFrame()
	...
	if (foundLayer or (NWB.faction == "Horde" and zone == 1454)
		...
		if (NWB.currentLayer > 0) then
			MinimapLayerFrame.fs:SetText(L["Layer "] .. NWB.lastKnownLayer);
			MinimapLayerFrame.fs:SetFont(NWB.regionFont, 12);
		elseif (layerNum > 0) then
			MinimapLayerFrame.fs:SetText(L["Layer "] .. layerNum);
			MinimapLayerFrame.fs:SetFont(NWB.regionFont, 12);
		else
